...
dataset.fillna(0, inplace = True)#replaces the NULL values with a specified value
...
def PredictCropPricesAction(request):
    if request.method == 'POST':
        ...
        #print("Accuracy of knn algo", knn_regression.accuracy_score(X_test, Y_test) * 100)
...
def getOutput(table,length):
    ...
    output=""
    con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'Manasvini@2001', database = 'cropinfo',charset='utf8')
    with con:
        cur = con.cursor()
        ...
        with con:
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break
        if output == 'none':
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'Manasvini@2001', database = 'cropinfo',charset='utf8')
